...
DOUBLE PRECISION FUNCTION halobias(lnnu, delta)
!!$ Halo bias
	IMPLICIT none
	DOUBLE PRECISION, intent(IN) :: lnnu, delta
	DOUBLE PRECISION :: q=0.707d0,p=0.3d0, nu
	nu = exp(lnnu)
	halobias = 1d0 + ((q*nu - 1d0)/delta) + ((2d0*p/delta) / (1 + (q*nu)**p))
	return
END FUNCTION halobias
