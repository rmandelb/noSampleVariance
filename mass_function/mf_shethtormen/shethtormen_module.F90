...
function execute(block, config) result(status)
	...
	real(dl), dimension(:,:), allocatable ::  dm, dr, db
	real(dl) :: deltam
...
!	! mass bin sizes  in compute_mf_pressschechter.f90
	...
	if (status .ne. 0) then
        ...
        if(settings%redshift_zero == 1) then
                ...
                status = datablock_put_double_array_1d(block,mass_function_section, "b_h",MassF%bias)
        ...
        if(settings%redshift_zero == 0) then
                ...
                status = datablock_put_double_array_2d(block,mass_function_section,"b_h", db)
        end if
